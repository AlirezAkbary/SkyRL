...
preparation log, full training log, and a compact `probe.log` under
...
- `RESULT=INCONCLUSIVE`: training failed to start/finish or the probe never
  reached the post-training forward. Read `train.log` for the first error.
...
```bash
MODEL=/path/to/local/model STEPS=5 bash examples/train/fully_async/repro_issue_2247.sh
```
